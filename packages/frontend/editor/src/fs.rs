//! Thin wrapper over the File System Access API (ported from awsm-renderer).
//!
//! Directory-based projects are Chromium-only by necessity (this API). A
...
// Some helpers (text read, existence) round out the surface ahead of every
// caller landing.
#![allow(dead_code)]
...
impl ProjectDir {
    ...
    /// Ensure we still have readwrite permission. Re-prompts if the browser has
    /// downgraded us since the initial pick.
    ...
    /// Walk a forward-slash-joined path, optionally creating missing directories,
    /// returning the final `FileSystemFileHandle`.
