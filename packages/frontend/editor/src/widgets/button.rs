//! Buttons — `Btn` (ghost / quiet / primary / solid / danger, sm/md/lg) and
//! `IconBtn` (a square icon action with optional label + active/danger states),
//! ported from the awsm-renderer atoms. Hover is a per-button `Mutable<bool>` so
//! the background + text color flip together (a CSS `:hover` can't express the
//! variant-specific color pairs as cleanly).
...
pub enum BtnVariant {
    ...
    Ghost,
    /// Transparent, no outline — low-emphasis inline actions.
    Quiet,
    /// Accent-filled primary call-to-action.
    Primary,
    /// Filled neutral (elevated surface) with a line border.
    Solid,
    /// Danger-tinted ghost (destructive actions).
    Danger,
}
...
fn variant_style(v: BtnVariant) -> VariantStyle {
    match v {
        ...
        BtnVariant::Danger => VariantStyle {
            bg: "transparent",
            bg_hover: "var(--danger-soft)",
            fg: "var(--danger-bright)",
            fg_hover: "var(--danger-bright)",
